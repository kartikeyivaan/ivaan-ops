...
import { listAllTasks } from "@/lib/task-service";
import { taskListQuerySchema } from "@/lib/task-validations";

export async function GET(request: Request) {
  const session = await auth();
  if (!session?.user || !canViewAllTasks(session.user.roles)) {
  ...
  try {
    const result = await listAllTasks(prisma, taskActorFromSession(session), parsed.data);
