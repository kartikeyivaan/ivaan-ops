...
import { reassignTask } from "@/lib/task-service";
import { reassignTaskSchema } from "@/lib/task-validations";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = reassignTaskSchema.safeParse(await request.json());
  if (!parsed.success) {
    return taskError("VALIDATION_ERROR", "Select an assignee.", 400, parsed.error.flatten());
  }

  try {
    const { id } = await context.params;
    const task = await reassignTask(
      prisma,
