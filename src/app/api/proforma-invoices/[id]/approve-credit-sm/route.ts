...
import { approvePiCreditSm } from "@/lib/pi-credit-service";
import { canApprovePiCreditSm } from "@/lib/pi-permissions";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  if (!session?.user || !canApprovePiCreditSm(session.user.roles)) {
  ...
  try {
    const credit = await approvePiCreditSm(prisma, {
      companyId,
      piId: id,
    ...
    if (error instanceof Error) {
      ...
      if (error.message === "NO_PENDING_CREDIT_SM") {
        return errorResponse(
          "NO_PENDING_CREDIT_SM",
          "No pending Sales Manager credit approval for this PI.",
          400,
        );
      }
