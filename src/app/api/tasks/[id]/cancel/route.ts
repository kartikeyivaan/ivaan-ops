...
import { cancelTaskSchema } from "@/lib/task-validations";
...
export async function POST(request: Request, context: RouteContext) {
  ...
  const parsed = cancelTaskSchema.safeParse(await request.json());
  if (!parsed.success) {
    return taskError("VALIDATION_ERROR", "A cancellation reason is required.", 400, parsed.error.flatten());
  }

  try {
    const { id } = await context.params;
    const task = await cancelTask(prisma, taskActorFromSession(session), id, parsed.data.reason);
